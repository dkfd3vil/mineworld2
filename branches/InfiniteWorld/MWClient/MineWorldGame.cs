...
namespace MineWorld
{
    public class MineWorldGame : StateMachine
    {
        ...
        public void JoinGame(IPEndPoint serverEndPoint)
        {
            ...
            NetBuffer connectBuffer = PropertyBag.Client.CreateBuffer();
            connectBuffer.Write(Defines.MineworldBuild);
            connectBuffer.Write(PropertyBag.PlayerHandle);

            // Connect to the server.
            PropertyBag.Client.Connect(serverEndPoint,connectBuffer);
        }
        ...
        public List<ServerInformation> EnumerateServers(float discoveryTime)
        {
            ...
            PropertyBag.Client.DiscoverLocalServer(Defines.MineworldPort);
            NetBuffer msgBuffer = PropertyBag.Client.CreateBuffer();
            float timeTaken = 0;
            while (timeTaken < discoveryTime)
            {
                NetMessageType msgType;
                while (PropertyBag.Client.ReadMsg(msgBuffer, out msgType))
                {
        ...
        public void UpdateNetwork(GameTime gameTime)
        {
            ...
            NetMessageType msgType;
            while (PropertyBag.Client.ReadMsg(_msgBuffer, out msgType))
            {
                switch (msgType)
                {
                    case NetMessageType.StatusChanged:
                        {
                            if (PropertyBag.Client.GetStatus() == NetConnectionStatus.Disconnected)
                            {
                                ...
                                case MineWorldMessage.BlockBulkTransfer:
                                    {
                                        ...
                                        mpg.drawCube(x, y, z, x, y / 2, z, BlockType.Dirt);
                                        PropertyBag.BlockEngine.SetBlockList(mpg.mapData, x, y, z);
        ...
        protected override void Initialize()
        {
            ...
            if (dataFile.Data.ContainsKey("width"))
                Csettings.Width = int.Parse(dataFile.Data["width"]);
            if (dataFile.Data.ContainsKey("height"))
                Csettings.Height = int.Parse(dataFile.Data["height"]);
            ...
            if (dataFile.Data.ContainsKey("sensitivity"))
                Csettings.MouseSensitivity = Math.Max(0.001f,
                                                      Math.Min(0.05f,
                                                               float.Parse(dataFile.Data["sensitivity"])/1000f));
             */
        ...
        {
            PropertyBag.Client.Shutdown("Client exiting.");

            base.OnExiting(sender, args);
        }

        public void InitializePropertyBag()
        {
            if (PropertyBag != null)
                PropertyBag.Client.Shutdown();
            ...
            _msgBuffer = PropertyBag.Client.CreateBuffer();
        }

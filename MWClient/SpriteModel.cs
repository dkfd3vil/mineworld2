...
namespace MineWorld
{
    ...
    {
        private readonly string _animationEvent;
    ...
    public class SpriteModel
    {
        ...
        private List<AnimationFrame> _activeAnimation;
        private int _animationStep;
        private int _currentColumn;
        private List<AnimationFrame> _passiveAnimation;
        private bool _runningActive;
        private Texture2D _texSprite;
        ...
        public SpriteModel(Game gameInstance)
        {
            _graphicsDevice = gameInstance.GraphicsDevice;
            _effect = gameInstance.Content.Load<Effect>("effect_spritemodel");
            _nameFont = gameInstance.Content.Load<SpriteFont>("font_04b08");
            _texSprite = gameInstance.Content.Load<Texture2D>("sprites/tex_sprite_blue_construction");
            _texSprite = gameInstance.Content.Load<Texture2D>("sprites/tex_sprite_blue_construction");

            AnimationFrame dummyFrame = new AnimationFrame();
            dummyFrame.Length = 1;
            dummyFrame.SpriteColumn = 1;
            _passiveAnimation = new List<AnimationFrame>();
            _passiveAnimation.Add(dummyFrame);
            _activeAnimation = new List<AnimationFrame>();
            _activeAnimation.Add(dummyFrame);
        }

        public void SetSpriteTexture(Texture2D spriteTexture)
        {
            _texSprite = spriteTexture;
        }
        ...
        public void Draw(Matrix viewMatrix, Matrix projectionMatrix, Vector3 cameraPosition, Vector3 cameraForward,
                         Vector3 drawPosition, Vector3 drawHeading, float drawScale)
        {
            ...
            Matrix world = Matrix.CreateConstrainedBillboard(drawPosition, cameraPosition, Vector3.UnitY, cameraForward,
                                                             null);
            ...
            _effect.Parameters["xTexture"].SetValue(_texSprite);
            _effect.Begin();
            ...
            _graphicsDevice.RenderState.ReferenceAlpha = 128;
                             ...
                             Color color)
        {
            ...
            Vector3 screenSpace = _graphicsDevice.Viewport.Project(Vector3.Zero,
                                                                  projectionMatrix,
                                                                  viewMatrix,
                                                                  Matrix.CreateTranslation(drawPosition +
                                                                                           new Vector3(0, 1.7f, 0)));
            Vector2 textPosition = new Vector2(screenSpace.X, screenSpace.Y) - _nameFont.MeasureString(hoverText)*0.5f;
            textPosition.X = (int) textPosition.X;
            textPosition.Y = (int) textPosition.Y;
            spriteBatch.DrawString(_nameFont, hoverText, textPosition, Color.Black);
            textPosition.X -= 2;
            textPosition.Y -= 2;
            spriteBatch.DrawString(_nameFont, hoverText, textPosition, color); //Color.White);
            spriteBatch.End();
        }

        public VertexPositionTexture[] GenerateVertices(Vector3 cameraPosition, Vector3 drawPosition,
                                                        Vector3 drawHeading, float drawScale)
        {
            VertexPositionTexture[] vertices = new VertexPositionTexture[6];

            float frameWidth = 24.0f/_texSprite.Width;
            float frameHeight = 32.0f/_texSprite.Height;
            ...
            float texX = _currentColumn*frameWidth - frameWidth/24;
            float texY = spriteRow*frameHeight + frameHeight/32;
            VertexPositionTexture v1 = new VertexPositionTexture(new Vector3(-0.375f*drawScale, 1*drawScale, 0),
                                                                 new Vector2(texX, texY));
            VertexPositionTexture v2 = new VertexPositionTexture(new Vector3(0.375f*drawScale, 1*drawScale, 0),
                                                                 new Vector2(texX + frameWidth, texY));
            VertexPositionTexture v3 = new VertexPositionTexture(new Vector3(-0.375f*drawScale, 0, 0),
                                                                 new Vector2(texX, texY + frameHeight));
            VertexPositionTexture v4 = new VertexPositionTexture(new Vector3(0.375f*drawScale, 0, 0),
                                                                 new Vector2(texX + frameWidth, texY + frameHeight));
        ...
        {
            List<AnimationFrame> currentAnimation = _runningActive ? _activeAnimation : _passiveAnimation;

            _timeCountdown -= (float) gameTime.ElapsedGameTime.TotalSeconds;
            if (_timeCountdown <= 0)
            {
                _animationStep += 1;
                if (_animationStep >= currentAnimation.Count)
                {
                    // The animation has run over, so reset it.
                    if (_runningActive)
                    {
                        _runningActive = false;
                        currentAnimation = _passiveAnimation;
                    }
                    _animationStep = 0;
                }

                _timeCountdown = currentAnimation[_animationStep].Length;
                _currentColumn = currentAnimation[_animationStep].SpriteColumn;
            }
        }
        ...
        {
            _passiveAnimation = ParseAnimationScript(animationScript);
        }
        ...
        {
            _activeAnimation = ParseAnimationScript(animationScript);
            _runningActive = true;
            _timeCountdown = _activeAnimation[0].Length;
            _currentColumn = _activeAnimation[0].SpriteColumn;
        }

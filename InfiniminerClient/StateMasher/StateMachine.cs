...
namespace StateMasher
{
    ...
    public class StateMachine : Microsoft.Xna.Framework.Game
    {
        ...
        public Color calculateSkyColor(float time)
        {
        ...
        public byte calculateGradientPoint(byte value1, byte value2, float position)
        {
        ...
        protected override void Draw(GameTime gameTime)
        {
            if (propertyBag != null)
            {
                GraphicsDevice.Clear(calculateSkyColor(propertyBag.time));
            }
            else
            {
                //fallback
